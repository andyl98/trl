...
def compute_logps_with_prompt_cache(
    ...
    # TODO: perhaps we can shrink the peak memory further here by using mini_batch, but the Past KV is hard to slice
    ...
    # Use the mini_batch_size to get the completion_logps; if mini_batch_size unspecified, defaults to entire batch
    if mini_batch_size <= 0:
        mini_batch_size = completion_logits.size(0)

    per_token_logps = []
    for i in range(0, completion_logits.size(0), mini_batch_size):
        mini_batch_log_probs = completion_logits[i : i + mini_batch_size, :, :].log_softmax(
            dim=-1
        )  # (B_mini, C-1, vocab_size)
        mini_batch_token_index = completion_ids[i : i + mini_batch_size, 1:].unsqueeze(-1)  # (B_mini, C-1, 1)

        mini_batch_token_log_prob = torch.gather(mini_batch_log_probs, dim=-1, index=mini_batch_token_index).squeeze(
            -1
        )
    ...
    per_token_logps = torch.cat([first_completion_token_logps, per_token_logps], dim=1)
